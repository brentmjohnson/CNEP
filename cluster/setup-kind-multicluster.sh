...
# default_kind_node_version=v1.24.3
default_kind_node_version=v1.23.6
...
function create_registry() {
  ...
  if [ "${running}" != 'true' ]; then
    docker run -d --restart=always -p "${registry_port}:5000" --name "${registry_name}" -e REGISTRY_STORAGE_DELETE_ENABLED=TRUE registry:2
  fi
}
...
- role: control-plane
  ...
  - containerPort: 30942
    ...
    protocol: TCP
  - containerPort: 30992
    hostPort: 3${cluster_id}992
    protocol: TCP
  - containerPort: 30920
    hostPort: 3${cluster_id}920
    protocol: TCP
