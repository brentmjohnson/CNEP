...
local schema = {
    ...
    properties = {
        ...
        --     type = "object",
        --     default = {},
        --     description = "additional authorization parameters to be passed to the " ..
...
function _M.rewrite(plugin_conf, ctx)
    ...
    if conf.include_original_request_uri then
        -- conf.authorization_params.original_request_uri = ctx.var.request_uri

        -- Check if original_request_uri already exists in the query string
        local original_request_uri = ctx.var.query_string 
            and ngx.decode_args(ctx.var.query_string).original_request_uri 
            or ctx.var.request_uri
        original_request_uri = ngx.encode_args({ original_request_uri = original_request_uri })
        
        -- Check if the redirect_uri already has query parameters
        local separator = "?"
        if string.find(conf.redirect_uri, "?", 1, true) then
            separator = "&"
        end

        -- Check if the redirect_uri has a hash
        local hash_position = string.find(conf.redirect_uri, "#", 1, true)
        if hash_position then
            -- Append the original_request_uri parameter before the hash symbol
            conf.redirect_uri = string.sub(conf.redirect_uri, 1, hash_position - 1) .. separator .. original_request_uri .. string.sub(conf.redirect_uri, hash_position)
        else
            -- Append the original_request_uri parameter at the end of the redirect_uri
            conf.redirect_uri = conf.redirect_uri .. separator .. original_request_uri
        end
    end
